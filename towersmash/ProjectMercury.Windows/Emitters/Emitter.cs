...
namespace ProjectMercury.Emitters
{
    ...
    public class Emitter
    {
        ...
        protected void CopyBaseFields(Emitter emitter)
        {
            ...
            emitter.ParticleTextureAssetName = (string)this.ParticleTextureAssetName.Clone();
        ...
        public unsafe void Update(float deltaSeconds)
        {
            ...
            for (int i = 0; i < this.Idle; i++)
            {
                ...
                {
                    // Calculate the actual age of the particle in whole and fractional seconds...
                    float actualAge = this.TotalSeconds - particle->Inception;

                    if (checkAge)
                    {
                        // If its age is > Emitter term, we need not do any further processing on it, as it will be retired...
                        if (actualAge > this.Term)
                        {
                            // Raise the ParticleRetired event...
                            this.OnParticleRetired(ref *particle);
                        ...
                        {
                            // If we find a Particle whose age is within Emitter term, we know the following
                            // Particles will be of equal age or younger, so for the remainder of this loop
                            // we no longer need to check for expired Particles...
                            checkAge = false;
                        }
                    }

                    // Calculate the age of the Particle in the range 0 to 1...
                    particle->Age = actualAge / this.Term;

                    // Send the active Particle to any Modifiers acting upon the Emitter...
                    this.Modifiers.RunProcessors(deltaSeconds, particle, this.Tags[i]);
            ...
            for (int i = oldIdle; i < oldIdle + this.ReleaseQuantity; i++)
            {
                if (i < this.Budget)
                {
                    fixed (Particle* particle = &this.Particles[i])
                    {
                        ...
                        particle->Rotation   = this.ReleaseRotation.Sample();
