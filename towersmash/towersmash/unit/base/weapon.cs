...
namespace towersmash
{
    ...
    public class weapon
    {
        ...
        public void shoot(Vector2 worldLocationofTarget)
        {
            if (isTimer)
            {
                this.direction = worldLocationofTarget;
                master.shootbullet(location, worldLocationofTarget, this.w_bullet);
            }
        }

        public void Shoot(Vector2 startingPosition, Vector2 aimDirection)
        {
            if(isTimer)
            {
                master.ShootBullet(w_bullet, startingPosition, aimDirection);
            }
        }
